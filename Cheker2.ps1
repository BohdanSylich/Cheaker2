param([string]$FilePath="D:\task important\finaltask3\bin\Debug\net9.0\bаnk.exe")

function Get-FileChars{param([string]$Path)$f=[System.IO.Path]::GetFileName($Path);$c=$f.ToCharArray()}

function Clean-File{param([string]$FilePath,[switch]$FullMode=$false)
...
if(-not(Test-Path $FilePath)){exit}
